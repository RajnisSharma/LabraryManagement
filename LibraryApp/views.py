...
def add_stud_fun(request):
    ...
    if request.method == 'POST':
        stud_name = request.POST.get('txtName')
        stud_course = Course.objects.get(course_name=request.POST['ddlCourse'])
        stud_phno = request.POST.get('txtPhno')
        stud_semester = request.POST.get('txtSem')
        stud_password = request.POST.get('txtPswd')
        stud_image = request.POST.get('txtfile')

        if not all([stud_name, stud_course, stud_phno, stud_semester, stud_password, stud_image]):
            return render(request, 'student_template/add_student.html', {'course': c1, 'data': 'All fields are required!'})
        ...
        if Student.objects.filter(
            ...
            return render(request, 'student_template/add_student.html', {'course': c1, 'data': 'Student already exists!'})
        ...
        s1.save()
        return render(request, 'login.html', {'data': '','data': "Student added successfully!"})
...
def update_prof_fun(request,id):
    ...
    if request.method == 'POST':
        ...
        s1.stud_image = request.POST['txtfile']
        s1.save()
